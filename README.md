...
Daily aggregator of Louisiana-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "LA".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, LA` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Louisiana"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/louisiana/`
...
```html
<iframe
  src="https://taildraggers.github.io/louisiana/"
  title="Taildragger Ads in Louisiana"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_LOUISIANA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
