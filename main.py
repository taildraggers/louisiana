"""Aggregates Louisiana-located listings from every taildraggers.com
...
ones whose location ends in ", LA".
...
Indiana/Iowa/Kansas/Kentucky repos' approach, with the state filter swapped
to "LA".
"""
...
PAGE_TITLE = "Taildragger Ads in Louisiana"
STATE_ABBR = "LA"
...
_LOUISIANA_LOCATION_RE = re.compile(r",\s*LA\s*$", re.IGNORECASE)


def _is_louisiana(location: str) -> bool:
    return bool(_LOUISIANA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        louisiana_rows = [row for row in rows if _is_louisiana(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(louisiana_rows)} in Louisiana")
        all_listings.extend(louisiana_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Louisiana listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Louisiana listings: {len(listings)}")
